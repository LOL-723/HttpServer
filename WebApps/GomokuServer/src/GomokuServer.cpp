...
void GomokuServer::getBackendData(const http::HttpRequest &req, http::HttpResponse *resp)
{
    try 
    {
        ...
        LOG_INFO ("当前在线人数: %d",curOnline);
        
        int maxOnline = getMaxOnline();
        LOG_INFO ("历史最高在线人数: %d",maxOnline);
        
        int totalUser = getUserCount();
        LOG_INFO("已注册用户总数: %d",totalUser);
    ...
    {
        LOG_ERROR ("Error in getBackendData: %s",e.what());
                             ...
                             http::HttpResponse *resp)
{
    ...
    {
        LOG_ERROR ("Error in packageResp: %d",e.what());
        // 设置一个基本的错误响应
        resp->setStatusCode(http::HttpResponse::k500InternalServerError);
        resp->setStatusMessage("Internal Server Error");
        resp->setCloseConnection(true);
