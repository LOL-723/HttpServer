#include "../../../include/httpserver/http/HttpContext.h"
...
bool HttpContext::processRequestLine(const char *begin, const char *end){
    ...
    if(space!=end && request_.setMethod(start,space)){
        ...
        if(space!=end){
            const char*aQuestionMarkWTF=std::find(start,space,'?');
            if(aQuestionMarkWTF!=space){
                request_.setPath(start, aQuestionMarkWTF);// 注意这些返回值边界
                request_.setQueryParameters(aQuestionMarkWTF+1, space);
            }else{// 请求不带参数
