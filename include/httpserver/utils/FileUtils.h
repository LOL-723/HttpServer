...
public:
    ...
    {
        file_.close();
        file_.open(std::string(HTTP_SERVER_PROJECT_ROOT) + "/WebApps/GomokuServer/resource/NotFound.html", std::ios::binary);
    }
    ...
    void readFile(std::vector<char>& buffer)
    {
        ...
        {
            LOG_INFO("File content load into memory (%llu bytes)", size());
        }    
        else
        {
